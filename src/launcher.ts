  ...
  async run() {
    return new Promise(
        async (
          ...
          if (!toolkitDitExists) {
            const error =
                new Error(`Cannot launch ${this.id}, directory not found.`);
            reject(error);
            return;
          }
          ...
          if (!packageJsonExists) {
            const error =
                new Error(`Cannot launch ${this.id}, package.json not found.`);
            reject(error);
            return;
          }

          const packageJson = require(packageJsonPath) as
              {main: string, displayName: string, view: string};
          ...
          if (!entryExists) {
            const error =
                new Error(`Cannot launch ${this.id}, entry file not found.`);
            reject(error);
            return;
          }
          ...
          if (!packageJson.view) {
            const error =
                new Error(`Cannot launch ${this.id}, view path not found.`);
            reject(error);
            return;
          }
